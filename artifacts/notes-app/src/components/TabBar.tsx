    ...
    <Draggable draggableId={`tab-${id}`} index={index}>
      {(provided) => (
        ...
        >
          ...
          {note?.isLocked && <span className="text-xs opacity-60">🔒</span>}
...
export default function TabBar({ selectedFolderId, onCreateNote }: TabBarProps) {
  const { openTabs, reorderTabs } = useTabs();
